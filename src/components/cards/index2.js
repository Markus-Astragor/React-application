...
function Main_page() {
  ...
  .filter(card => tag == null || card.topic === tag)
  .map(
    card => <Card
      image={card.image}
      title={card.title}
  ...
  let myclassName = ['selected_tag'];
  if(tag){
    myclassName.push('show')
  }
  return (
    <div className="right-side">
     {tag && <div className={myclassName.join(' ')}>
        {tag}
      </div>}
      {cardArray}
      <br />
      <button id="all_posts" onClick={()=> setTag()}>All posts-&#62;</button>
    </div>
  )
}
